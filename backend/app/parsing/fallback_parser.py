...
SIGNATURE_PATTERNS = [
    ...
    re.compile(
        r"^(?:export\s+)?(?:default\s+)?(?:async\s+)?function\s+(?P<name>[a-zA-Z0-9_$]+)",
        re.MULTILINE,
    ),
    re.compile(
        r"^(?:export\s+)?(?:const|let|var)\s+(?P<name>[a-zA-Z0-9_$]+)\s*=\s*(?:async\s*)?\([^)]*\)\s*=>",
        re.MULTILINE,
    ),
    re.compile(r"^(?:export\s+)?(?:abstract\s+)?class\s+(?P<name>[a-zA-Z0-9_$]+)", re.MULTILINE),
    re.compile(r"^(?:export\s+)?interface\s+(?P<name>[a-zA-Z0-9_$]+)", re.MULTILINE),
    # Go func
    re.compile(r"^func\s+(?:\([^)]+\)\s+)?(?P<name>[a-zA-Z0-9_]+)\s*\(", re.MULTILINE),
    # Rust fn & struct/impl
    re.compile(r"^(?:pub\s+)?(?:async\s+)?fn\s+(?P<name>[a-zA-Z0-9_]+)", re.MULTILINE),
    re.compile(r"^(?:pub\s+)?(?:struct|enum|trait|impl)\s+(?P<name>[a-zA-Z0-9_]+)", re.MULTILINE),
]
