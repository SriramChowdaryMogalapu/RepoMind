...
class ChatService:
    ...
    def _build_github_url(
        self, repo: Repository, file_path: str, start_line: int, end_line: int
    ) -> str:
        ...
        return (
            f"https://github.com/{repo.owner}/{repo.name}/blob/{branch}/{clean_path}{line_fragment}"
        )
    ...
    async def answer_question(self, repository_id: UUID, request: ChatRequest) -> ChatResponse:
        ...
        if request.tagged_files:
            logger.info(
                f"Injecting {len(request.tagged_files)} tagged files: {request.tagged_files}"
            )
            tagged_chunks = await self.retriever.get_tagged_file_chunks(
                repository_id=repository_id, file_paths=request.tagged_files
            )

        # 2. Perform hybrid retrieval for related context
        retrieved_candidates = await self.retriever.retrieve(
            repository_id=repository_id, query=request.question, top_k=request.top_k
        )
