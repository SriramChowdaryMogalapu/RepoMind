...
"""
    ...
    print(
        f"[INTEGRATION TEST] Stage 2 - Extracted {len(raw_chunks)} structural chunks from source code."
    )
    ...
    print(
        f"[INTEGRATION TEST] Stage 3 - Generated {len(embeddings)} batch embeddings (Dim: {len(embeddings[0])})."
    )
